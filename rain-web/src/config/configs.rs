...
impl DatabaseConfig {
    /// 初始化数据库连接池
    pub async fn init(config: &DatabaseConfig) -> Result<Arc<Pool<MySql>>> {
...
fn get_config_dir() -> Result<PathBuf> {
    let base_path: PathBuf = current_dir().context("无法确定当前目录")?;

    let mut config_dir: PathBuf = base_path.join(CONFIG_PATH);
...
impl Configs {
    /// 初始化配置文件
    pub fn init_config() -> Result<Arc<Configs>> {
        // 加载环境变量
        dotenv::dotenv().ok();
